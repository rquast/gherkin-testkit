...
export const ensureFeatureFileAndStepDefinitionScenarioHaveSameSteps = (
  ...
  if ((parsedScenario as ParsedScenarioOutline).scenarios) {
    const parsedScenarioOutlineScenarios = (parsedScenario as ParsedScenarioOutline)
      .scenarios;
  ...
  if (parsedStepCount !== stepDefinitionCount && options.errors) {
    ...
    errors.push(
      `Scenario "${parsedScenario.title}" has ${parsedStepCount} step(s) in the feature file, but ${stepDefinitionCount} step definition(s) defined.`
    );
  }
  ...
  parsedScenarioSteps.forEach((parsedStep, index) => {
    ...
    if (
      ...
      !matchSteps(parsedStep.stepText, stepFromStepDefinitions.stepMatcher)
    ) {
      // tslint:disable-next-line:max-line-length
      errors.push(
        `Expected step #${index + 1} in scenario "${
          parsedScenario.title
        }" to match "${parsedStep.stepText}".`
      );
    }
  });
