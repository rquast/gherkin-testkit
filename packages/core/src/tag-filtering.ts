...
const checkIfScenarioMatchesTagFilter = (
  ...
  if (!tagFilterFunction) {
    tagFilterFunction = convertTagFilterExpressionToFunction(
      tagFilterExpression
    );
...
export const applyTagFilters = (parsedFeature: ParsedFeature) => {
  ...
  const scenarios = parsedFeature.scenarios.map((scenario) =>
    setScenarioSkipped(parsedFeature, scenario)
  );
  const scenarioOutlines = parsedFeature.scenarioOutlines.map(
    (scenarioOutline) => {
      return {
        ...setScenarioSkipped(parsedFeature, scenarioOutline),
        scenarios: scenarioOutline.scenarios.map((scenario) =>
          setScenarioSkipped(parsedFeature, scenario)
        )
      };
    }
